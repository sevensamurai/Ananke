...
[TestFixture]
public sealed class AgentRuntimeWorkflowHostTests
{
    ...
    private static AgentRuntimeCredentialProvider MakeCredProvider() =>
    ...
    public async Task Start_adds_to_active_list()
    {
        ...
        var deployer = new AgentRuntimeDeployer(MakeCredProvider(), registry);
    ...
    {
        private readonly AgentRuntimeWorkflowHost _inner;
        public TestableHost(AgentRuntimeDeployer deployer, WorkflowManifest manifest, ToolKit toolKit)
            => _inner = new AgentRuntimeWorkflowHost(deployer, manifest, toolKit);
