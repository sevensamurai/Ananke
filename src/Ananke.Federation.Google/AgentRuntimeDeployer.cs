...
/// <list type="bullet">
///   <item>Model selection via <see cref="AgentRuntimeModelMapper"/></item>
...
/// </list>
/// </remarks>
public sealed class AgentRuntimeDeployer : IFederationDeployer
{
    ...
    private readonly AgentRuntimeModelMapper _modelMapper;
    ...
    /// </summary>
    public AgentRuntimeDeployer(
        AgentRuntimeCredentialProvider credentialProvider,
        IDeploymentRegistry deploymentRegistry,
        AgentRuntimeModelMapper? modelMapper = null,
    ...
    /// </summary>
    internal AgentRuntimeDeployer(
        IFederationCredentialProvider credentialProvider,
        IDeploymentRegistry deploymentRegistry,
        AgentRuntimeModelMapper? modelMapper,
        ...
        IAgentRuntimeClient? agentRuntimeClient)
    {
        ...
        _systemPromptCompiler = systemPromptCompiler ?? new AgentRuntimeSystemPromptCompiler();
    ...
    {
        var validator = new AgentRuntimeValidator(_credentialProvider, _modelMapper);
    ...
    {
        if (_credentialProvider is AgentRuntimeCredentialProvider p)
            return new AgentRuntimeClient(p.Project, p.Location);

        throw new InvalidOperationException(
            "Cannot construct AgentRuntimeClient: credential provider is not a AgentRuntimeCredentialProvider. " +
    ...
    private string ResolveModel(WorkflowManifest manifest, JobDefinition job)
    {
        ...
        return Models.Google.Gemini36Flash; // the current flash id the API answers to
    }
}
