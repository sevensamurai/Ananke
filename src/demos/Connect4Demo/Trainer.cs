    ...
    IExplorationStrategy? explorationStrategy = null)
{
    ...
    private async Task PrintTopPatternsAsync(List<EmpiricalEntry> entries)
    {
        var top = entries
    ...
    private sealed record ValidationResult(int Total, int Reinforced, int Contradicted, int Neutral, float BaselineWinRate);
}
