...
| `ANANKE_TIME_001` | Warning | A direct read of `DateTime.Now`/`UtcNow` or `DateTimeOffset.Now`/`UtcNow` anywhere in production code — inject a `TimeProvider` and call `GetUtcNow()` instead so the caller can substitute a `FakeTimeProvider` in tests. Suppressed for test/demo/CLI projects and a legacy per-file list in `src/.editorconfig`. |
