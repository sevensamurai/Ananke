...
# 00 — nnke Tool Companion

## Why We Created `nnke`

Starting with a new framework can be daunting.

Even when the documentation is good and the examples are there, you still have to figure out
where to start, which pattern fits your problem, how to shape the workflow, and how to move from
an idea to something runnable without wasting time on setup.

That is why we created `nnke`, the Ananke CLI.
It exists to make the first steps easier, shorten the path from idea to working project,
and give both humans and AI coding tools a practical companion while building with Ananke.

We created `nnke` for two practical reasons:

1. **To make quick starts much faster.** New users should be able to scaffold a project, generate a manifest,
	validate a topology, and get moving without first learning every Ananke concept in depth.
2. **To work well with AI-assisted coding.** Tools such as GitHub Copilot and Claude are much more useful when
	they can inspect a project, validate manifests, search docs, and return structured results instead of guessing.

In human terms, `nnke` exists to reduce friction.
It helps you go from idea to working shape faster, and it gives AI tools something concrete and machine-readable
to work with while you build.
...
- **Copilot / Claude** become more useful when they can call `nnke` or consume its `--json` output

`nnke` is a packaged .NET CLI tool.

It is published as a .NET tool package and installed like any other dotnet CLI tool.
Once installed, you run it from the terminal as:

```bash
nnke <command>
```

The tool is designed to help with workflow design and project diagnostics rather than runtime hosting.
...
- browse Ananke docs from the terminal
...
## What `nnke` Is Good At

`nnke` is most useful when you are working on workflow structure and project shape rather than only writing code.
A typical use case is:

1. sketch a workflow in natural language with Copilot or Claude
2. scaffold or write a manifest and topology
3. validate and refine the topology
4. inspect the project for missing pieces
5. bind concrete job implementations in C#
6. export diagrams or artifacts for review

This makes it a good fit for:
- early architecture and prototyping
- design reviews with humans or AI assistants
- keeping workflow topology separate from runtime behavior
- turning a rough prompt into a repeatable workflow definition
- giving agents machine-readable project context via `--json`
...
- [Welcome](../welcome.md)
- [Overview & Learning Path](../learning.md)
- [01 — Getting Started](01-getting-started.md)
- [13 — Design Tooling](13-design-tooling.md)
- [Workflow DSL](../reference/workflow-dsl.md)
- [12 — MCP & Interop](12-mcp-and-interop.md)
